const messages = {
  en: {
    ...
    "options.subtitle": "Configure Obsidian backend connection & Profiles",
    "api.title": "Obsidian Backend Config",
    "api.backendLabel": "Backend",
    "api.backend.localRest": "Local REST API",
    "api.backend.vaultEcho": "VaultEcho",
    "api.localRestLink": "Local REST API Plugin",
    "api.vaultEchoLink": "VaultEcho",
    "api.backendHint.localRest": 'Requires desktop Obsidian and the Local REST API plugin. You can also search "Local REST API" in the Obsidian plugin marketplace.',
    "api.backendHint.vaultEcho": "Uses the VaultEcho HTTP gateway and Obsidian Headless sync; desktop Obsidian does not need to stay open. Attachments use VaultEcho's configured attachment folders.",
    ...
    "api.keyPlaceholder": "Bearer token",
    "api.keyPlaceholder.localRest": "Get from Obsidian plugin settings",
    "api.keyPlaceholder.vaultEcho": "VaultEcho API_TOKEN",
    ...
    // Config Sync
    "sync.title": "Config Sync",
    "sync.desc": "Sync profiles and settings across Chrome browsers via your Google account. API URL and Key are always local.",
    "sync.pushBtn": "Push to Cloud",
    "sync.pullBtn": "Pull from Cloud",
    "sync.pushOk": "Config pushed",
    "sync.pullOk": "Config pulled, page will reload",
  ...
  zh: {
    ...
    "options.subtitle": "配置 Obsidian 后端连接 & Profile",
    "api.title": "Obsidian 后端配置",
    "api.backendLabel": "后端",
    "api.backend.localRest": "Local REST API",
    "api.backend.vaultEcho": "VaultEcho",
    "api.localRestLink": "Local REST API 插件",
    "api.vaultEchoLink": "VaultEcho",
    "api.backendHint.localRest": '需要桌面端 Obsidian 正在运行，并启用 Local REST API 插件。也可在 Obsidian 插件市场搜索 "Local REST API" 安装。',
    "api.backendHint.vaultEcho": "通过 VaultEcho HTTP 网关和 Obsidian Headless 同步写入，桌面端 Obsidian 不需要常驻运行。附件使用 VaultEcho 服务端配置的附件目录。",
    "api.pluginLink": "Local REST API 插件",
    "api.pluginHint": '（也可在 Obsidian 插件市场搜索 "Local REST API" 安装）',
    "api.urlLabel": "API 地址",
    "api.keyLabel": "API Key",
    "api.keyPlaceholder": "Bearer token",
    "api.keyPlaceholder.localRest": "在 Obsidian 插件设置中获取",
    "api.keyPlaceholder.vaultEcho": "VaultEcho API_TOKEN",
    ...
    // 配置同步
    "sync.title": "配置同步",
    "sync.desc": "通过 Google 账号在多台 Chrome 浏览器之间同步 Profile 和设置。API 地址和 Key 始终使用本地配置。",
    "sync.pushBtn": "推送到云端",
    "sync.pullBtn": "从云端拉取",
    "sync.pushOk": "配置已推送",
    "sync.pullOk": "配置已拉取，页面将刷新",
    "sync.pushFail": "推送失败: ",
    "sync.pullFail": "拉取失败: ",
